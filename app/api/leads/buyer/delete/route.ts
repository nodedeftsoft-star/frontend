...
export async function DELETE(req: NextRequest) {
  try {
    ...
    const { leadBuyersIds } = body;

    //console.log("LEAD BUYER IDS", leadBuyersIds);

    if (!leadBuyersIds || !Array.isArray(leadBuyersIds)) {
      return NextResponse.json({ message: "Invalid request - buyerIds array required" }, { status: 400 });
    }

    // Delete each buyer
    const deletePromises = leadBuyersIds.map((id) =>
      api.delete(`/leads/buyers/${id}`, {
        headers: {
    ...
    console.error("Error deleting lead buyers:", error);
