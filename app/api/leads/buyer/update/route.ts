...
export async function POST(req: NextRequest) {
  try {
    ...
    const { _id, ...buyerDataWithoutId } = body;

    //console.log("ID:", _id);
    //console.log("BUYER DATA:", buyerDataWithoutId);

    const response = await api.put(`/leads/buyers/update/${_id}`, buyerDataWithoutId, {
      headers: {
    ...
    //     { message: response.data.message || "Failed to update buyer" },
    ...
    console.error("Error updating leads buyer:", error);
