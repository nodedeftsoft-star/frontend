...
export async function POST(req: NextRequest) {
  try {
    ...
    //console.log("LEADS seller ID:", _id);

    const response = await api.put(
      `/leads/sellers/update-signed-status/${_id}`,
      { isSigned },
      {
        headers: {
    ...
    if (response.status !== 201) {
      return NextResponse.json(
        { message: response.data.message || "Failed to sign leads seller" },
    ...
    console.error("Error signing leads seller:", error);
