...
export async function DELETE(req: NextRequest) {
  try {
    ...
    if (!userIds || !Array.isArray(userIds)) {
      return NextResponse.json({ message: "Invalid request - userIds array required" }, { status: 400 });
    }
    ...
    if (error instanceof Error) {
      return NextResponse.json({ message: "Failed to delete users", error: error.message }, { status: 400 });
    }
