...
export default function ViewMatchRentals({
  matchRentals,
  closeAll,
}: {
  matchRentals: matchData | undefined | null;
  closeAll: () => void;
}) {
  const [activityNote, setActivityNote] = useState("");
  const [isSubmittingActivity, setIsSubmittingActivity] = useState(false);
  const [matchRentalsData] = useState<matchData>(matchRentals as matchData);
  const [stage, setStage] = useState<string>(matchRentalsData?.stage || "");
  const [status, setStatus] = useState<string>(matchRentalsData?.status || "");
  ...
  useEffect(() => {
    if (matchRentals?.activityNotes) {
      setLocalActivityNotes(matchRentals.activityNotes);
    }
  }, [matchRentals]);
  ...
  async function submitActivity() {
    try {
      ...
      queryClient.invalidateQueries({ queryKey: ["match-rental", matchRentals?._id] });
  ...
  const handleUpdateStage = async (stage: string, skipConfirmation = false) => {
    console.log("STAGE:", stage, "STATUS:", status, "CONFIRM:", confirmInContract);
    ...
    try {
      ...
      const response = await fetch("/api/match/change-stage", {
        ...
        body: JSON.stringify({ matchId: matchRentals?._id, matchStage: stage }),
      });
      ...
      if (stage === "Closed") {
        // console.log("UPDATING LISTING STATUS TO IN CONTRACT:", matchRentals?.property?.id, stage);
        const response = await fetch("/api/listings/change-status", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
            propertyId: matchRentals?.property?.id,
        ...
        queryClient.invalidateQueries({ queryKey: ["listing-rental", matchRentals?.property?.id] });
  ...
  const handleUpdateStatus = async (status: string) => {
    try {
      ...
      const response = await fetch("/api/match/change-status", {
        ...
        body: JSON.stringify({ matchId: matchRentals?._id, matchStatus: status }),
      });
    ...
    <>
      <div className="flex justify-between  px-8">
        <div>
          ...
          {matchRentalsData?.property?.address && (
            <p className="text-light-text">
              {matchRentalsData?.property?.address.streetAddress}, {matchRentalsData?.property?.address.city},{" "}
              {matchRentalsData?.property?.address.state} {matchRentalsData?.property?.address.zipcode}
            </p>
          )}
        </div>
        <div className="flex gap-2 pr-12">
          ...
          <Button variant={"ghost"} size={"icon"} className="bg-transparent" onClick={closeAll}>
            <Link href={"/matches/rentals/" + matchRentalsData?._id}>
