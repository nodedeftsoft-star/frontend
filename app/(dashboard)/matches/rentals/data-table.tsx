...
async function fetchLeadsBuyers(params: FetchBuyersParams) {
  const response = await fetch("/api/match/renters", {
    method: "POST",
...
export function DataTable<TData, TValue>({ columns }: DataTableProps<TData, TValue>) {
  ...
  const storeFilters = useSortingFiltersStore((state) => state?.filters?.matchesRentals);
  ...
  const {
    ...
    queryKey: ["matchesRentals", debouncedSearch],
    queryFn: () =>
      fetchLeadsBuyers({
  ...
  useEffect(() => {
    if (allLeadsBuyerData) {
      allLeadsBuyerData.forEach((match: matchData) => {
        queryClient.setQueryData(["match-rental", match._id], match);
      });
    }
  ...
  const contactsTable = useReactTable({
    ...
    onSortingChange: (updaterOrValue) => {
      ...
      setSorting("matchesRentals", newSorting);
    },
    ...
    <div className="relative pb-[100px]">
      <div className="flex items-center justify-between mb-4 sm:sticky top-[1px] z-1 bg-white">
        <div className="flex gap-2">
          <Badge
            ...
            onClick={() => setStatusFilter("matchesRentals", "")}
          >
            All
          </Badge>
          <Badge
            ...
            onClick={() => setStatusFilter("matchesRentals", "Active")}
          >
            Active
          </Badge>
          <Badge
            ...
            onClick={() => setStatusFilter("matchesRentals", "Inactive")}
          >
            Inactive
          </Badge>
        </div>
        <div className="flex items-center gap-4">
          ...
          <Filters
            ...
            filters={[
              ...
              { value: "Deposit in", label: "Deposit In" },
            ...
            onFilterChange={(filter, checked) => {
              ...
              setTypeFilter("matchesRentals", newFilters);
            }}
            value={typeFilter}
          />
      ...
      <Dialog open={bulkDeleteOpen} onOpenChange={setBulkDeleteOpen}>
        <DialogContent showCloseButton={false} className="sm:w-[500px] rounded-lg">
          <DialogHeader>
            <DialogTitle>Delete {selectedCount === 1 ? "Rental Match" : "Rental Matches"}</DialogTitle>
          </DialogHeader>
          <DialogDescription>
            You&apos;re about to permanently delete {selectedCount} rental match{selectedCount === 1 ? "" : "es"} and
