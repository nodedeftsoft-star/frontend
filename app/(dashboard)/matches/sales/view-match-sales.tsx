...
export default function ViewMatchSales({
  matchSales,
  closeAll,
}: {
  matchSales: matchData | undefined | null;
  closeAll?: () => void;
}) {
  const [activityNote, setActivityNote] = useState("");
  const [isSubmittingActivity, setIsSubmittingActivity] = useState(false);
  const [matchSalesData] = useState<matchData>(matchSales as matchData);
  const [stage, setStage] = useState<string>(matchSalesData?.stage || "");
  const [status, setStatus] = useState<string>(matchSalesData?.status || "");
  ...
  const [localActivityNotes, setLocalActivityNotes] = useState<Array<{ note: string; timestamp: number }>>(
    matchSalesData?.activityNotes || []
  );

  const queryClient = useQueryClient();

  // Sync local notes when props change
  useEffect(() => {
    if (matchSales?.activityNotes) {
      setLocalActivityNotes(matchSales.activityNotes);
    }
  }, [matchSales]);
  ...
  async function submitActivity() {
    try {
      ...
      queryClient.invalidateQueries({ queryKey: ["match-sale", matchSales?._id] });
  ...
  const handleUpdateStage = async (stage: string, skipConfirmation = false) => {
    if (stage === "In contract" && confirmInContract === false && !skipConfirmation) {
    ...
    try {
      ...
      const response = await fetch("/api/match/change-stage", {
        ...
        body: JSON.stringify({ matchId: matchSales?._id, matchStage: stage }),
      });
      ...
      if (stage === "Closed") {
        const response = await fetch("/api/listings/change-status", {
          ...
          body: JSON.stringify({
            propertyId: matchSales?.property?.id,
        ...
        queryClient.invalidateQueries({ queryKey: ["listing-sale", matchSales?.property?.id] });
  ...
  const handleUpdateStatus = async (status: string) => {
    try {
      ...
      const response = await fetch("/api/match/change-status", {
        ...
        body: JSON.stringify({ matchId: matchSales?._id, matchStatus: status }),
      });
    ...
    <>
      <div className="flex justify-between  px-8">
        <div>
          <div className="flex items-center gap-4">
            <h1 className="text-light-text text-2xl font-bold">
              {matchSalesData?.customer?.firstName} {matchSalesData?.customer?.lastName}
            </h1>
            <span className="px-2 rounded-[8px] text-[#07192C] font-normal border border-[#E5E8EB] bg-[#D2E3F9]">
              Buyer
            </span>
          </div>
          {matchSalesData?.property?.address && (
            <p className="text-light-text">
              {matchSalesData?.property?.address?.streetAddress}, {matchSalesData?.property?.address?.city},{" "}
              {matchSalesData?.property?.address?.state} {matchSalesData?.property?.address?.zipcode}
            </p>
          )}
        </div>
        <div className="flex gap-2 pr-12">
          <Select
            value={stage}
            defaultValue={matchSalesData?.stage}
          ...
          >
            ...
            <SelectContent>
              <SelectGroup>
                ...
                <SelectItem value="Closing set">Closing Set</SelectItem>
          ...
          <Select
            value={status}
            defaultValue={matchSalesData?.status}
          ...
          <Button variant={"ghost"} size={"icon"} className="bg-transparent" onClick={closeAll}>
            <Link href={"/matches/sales/" + matchSalesData?._id}>
          ...
          <div className="w-full ">
            <div className="border border-[#E5E8EB] w-full flex flex-col rounded-lg p-6 mb-4 ">
              ...
              <div className="flex flex-col gap-4">
                {[
                  { label: "Type", value: "Sales Match" },
                  { label: "Email", value: matchSalesData?.customer.email },
                  { label: "Mobile", value: formatPhoneToInternational(matchSalesData?.customer.phoneNumber || "") },
                ].map(({ label, value }, idx) => (
            ...
            <div className="border border-[#E5E8EB] w-full flex flex-col rounded-lg p-6 mb-4 ">
              ...
              <div className="flex flex-col gap-4">
                {[
                  { label: "Name", value: matchSalesData?.property.attribution?.agentName },
                  {
                    label: "Contact",
                    value: formatPhoneToInternational(matchSalesData?.property?.attribution?.agentContactInfo || ""),
                  },
                ].map(({ label, value }, idx) => (
                  <div key={idx} className="flex items-center gap-4">
                    <p className="text-light-text-secondary text-sm w-32 shrink-0">{label}</p>
            ...
            <div className="border border-[#E5E8EB] w-full flex flex-col rounded-lg p-6 mb-4 ">
              ...
              <div className="flex flex-col gap-4">
                {[
                  { label: "Name", value: matchSalesData?.property?.attribution?.brokerName },
                  {
                    label: "Contact",
                    value: formatPhoneToInternational(matchSalesData?.property.attribution.brokerContactInfo || ""),
                  },
                ].map(({ label, value }, idx) => (
                  <div key={idx} className="flex items-center gap-4">
                    <p className="text-light-text-secondary text-sm w-32 shrink-0">{label}</p>
            ...
            <div className="border border-[#E5E8EB] w-full flex flex-col rounded-lg p-6 mb-4 ">
              ...
              <div className="flex flex-col gap-4">
                {[
                  {
                    label: "Address",
                    value: `${matchSalesData?.property?.address?.streetAddress}, ${matchSalesData?.property?.address?.city},  ${matchSalesData?.property?.address?.state} ${matchSalesData?.property?.address?.zipcode}`,
                  },
                  {
                    label: "Location",
                    value: matchSalesData?.property?.address?.parentRegionName,
                  },
                  {
                    label: "Type",
                    value: matchSalesData?.property?.homeType,
                  },
                  {
                    label: "Price",
                    value: matchSalesData?.property?.price?.toLocaleString("en-US", {
                      style: "currency",
                      currency: "USD",
                    }),
                  },
                  {
                    label: "Bedrooms",
                    value: matchSalesData?.property?.bedrooms,
                  },
                  {
                    label: "Bathrooms",
                    value: matchSalesData?.property?.bathrooms,
                  },
                ].map(({ label, value }, idx) => (
      ...
      <Dialog open={confirmInContract} onOpenChange={setConfirmInContract}>
        <DialogContent showCloseButton={false} className="sm:w-[1264px] rounded-t-lg rounded-b-lg">
          ...
          <div className="space-y-3">
            This listing&apos;s status will be changed to <span className="font-bold">In Contract</span> on Sales
          ...
          <DialogFooter>
            ...
            <Button
              ...
              onClick={() => handleUpdateStage("In contract", true)}
              disabled={isUpdatingStage}
            >
