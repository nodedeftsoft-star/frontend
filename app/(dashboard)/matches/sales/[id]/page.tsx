...
export default function MatchSalesDetails() {
  const { id } = useParams();
  ...
  const { data: matchSalesData, isLoading } = useQuery({
    queryKey: ["match-sale", id],
    queryFn: () => fetchMatchSales(id as string),
    enabled: !!id,
    // Use cached data immediately for instant display
    initialData: () => {
      return queryClient.getQueryData(["match-sale", id]);
    },
  ...
  useEffect(() => {
    if (matchSalesData) {
      setStage(matchSalesData.stage || "");
      setStatus(matchSalesData.status || "");
      setLocalActivityNotes(matchSalesData.activityNotes || []);
    }
  }, [matchSalesData]);
  ...
  async function submitActivity() {
    try {
      ...
      queryClient.invalidateQueries({ queryKey: ["match-sale", id] });
  ...
  const handleUpdateStage = async (stage: string, skipConfirmation = false) => {
    ...
    try {
      ...
      const response = await fetch("/api/match/change-stage", {
        ...
        body: JSON.stringify({ matchId: matchSalesData?._id, matchStage: stage }),
      });
      ...
      if (stage === "Closed") {
        const response = await fetch("/api/listings/change-status", {
          ...
          body: JSON.stringify({
            propertyId: matchSalesData?.property?.id,
        ...
        queryClient.invalidateQueries({ queryKey: ["listing-sale", matchSalesData?.property?.id] });
  ...
  const handleUpdateStatus = async (status: string) => {
    try {
      ...
      const response = await fetch("/api/match/change-status", {
        ...
        body: JSON.stringify({ matchId: matchSalesData?._id, matchStatus: status }),
      });
  ...
  const handleDeleteMatch = async () => {
    ...
    try {
      ...
      const response = await fetch("/api/match/delete", {
        ...
        body: JSON.stringify({ matchIds: [matchSalesData._id] }),
      });
    ...
    <>
      ...
      <Dialog open={deleteDialogOpen} onOpenChange={setDeleteDialogOpen}>
        <DialogContent showCloseButton={false} className="sm:w-[500px] rounded-lg">
          <DialogHeader>
            <DialogTitle>Delete Sales Match</DialogTitle>
          </DialogHeader>
          <DialogDescription>
            You&apos;re about to permanently delete this sales match and all associated information. This action cannot
            be undone.
            <br />
            <br />
      ...
      {/* In Contract Confirmation Dialog */}
