...
import { HandCoins, MapPinHouse } from "lucide-react";
  ...
  const menu = [
    {
      ...
      url: "/matches/sales",
      icon: HandCoins,
      isActive: pathname.startsWith("/matches/sales"),
    },
    {
      title: "Rentals",
      url: "/matches/rentals",
      icon: MapPinHouse,
      isActive: pathname.startsWith("/matches/rentals"),
    },
  ];

  return (
    <div className={`flex ${isMobile ? "flex-col" : "flex-row"} w-full min-h-screen h-full`}>
      <div
        className={`${
      ...
      <div className="flex-1 px-4  border-r-black border-r-2">
        <div className="py-4 text-neutral-400">
          <Breadcrumb>
            <BreadcrumbList>
              ...
              <BreadcrumbItem>
                <BreadcrumbLink href="/matches/sales">Matches</BreadcrumbLink>
              </BreadcrumbItem>
              {(pathname.includes("/sales") || pathname.includes("/rentals")) && (
                <>
                  <BreadcrumbSeparator className="hidden md:block" />
                  <BreadcrumbItem>
                    <BreadcrumbLink
                      href={pathname.includes("/sales") ? "/matches/sales" : "/matches/rentals"}
                      className="capitalize"
                    >
                      {pathname.includes("/sales") ? "Sales" : "Rentals"}
                    </BreadcrumbLink>
                  </BreadcrumbItem>
                </>
              )}
              {(pathname.includes("/[id]") || (/\/[^\/]+$/.test(pathname) && pathname.split("/").length > 3)) && (
                <>
                  <BreadcrumbSeparator className="hidden md:block" />
                  <BreadcrumbItem>
