...
async function fetchLeadsSeller(id: string) {
  ...
  //console.log("SELLER:", data.data.sellers[0]);
...
export default function ConvertSellerToListing() {
  ...
  const { data: leadsSeller, isLoading: isLoadingSeller } = useQuery({
  ...
  const form = useForm({
    ...
    defaultValues: {
      ...
      firstName: leadsSeller?.firstName || "",
      lastName: leadsSeller?.lastName || "",
      phoneNumber: leadsSeller?.phoneNumber || "",
      email: leadsSeller?.email || "",
      propertyCounty: leadsSeller?.propertyCounty || "",
      propertyPrice: leadsSeller?.propertyPrice || 0,
      propertyAddress: leadsSeller?.propertyAddress || "",
      propertyState: "NY",
      propertyZipcode: leadsSeller?.propertyZipcode || "",
      propertyNeighborhood: leadsSeller?.propertyNeighborhood || "",
      propertyURL: leadsSeller?.propertyURL || "",
      propertyType: leadsSeller?.propertyType || "",
      bedroomCount: leadsSeller?.bedroomCount || "",
      bathroomCount: leadsSeller?.bathroomCount || "",
      petOwned: leadsSeller?.petOwned || "no",
      ...
      amenities: leadsSeller?.amenities || [],
      description: leadsSeller?.description || "",
    },
  });
  ...
  useEffect(() => {
    if (leadsSeller) {
      ...
      if (leadsSeller?.propertyType) {
        //console.log("PROPERTY TYPE", leadsSeller?.propertyType);
        form.setValue("propertyType", leadsSeller.propertyType);
        //console.log("PROPERTY TYPE", form.watch("propertyType"));
      }
      if (leadsSeller?.petOwned) {
        //console.log("PPET OWNED", leadsSeller?.petOwned);
        form.setValue("petOwned", leadsSeller.petOwned);
      }
    }
  }, [leadsSeller]);
  ...
  async function onSubmit(data: z.infer<typeof formSchema>) {
    try {
      ...
      const encodedData = {
        ...
        salesNote: leadsSeller?.activityNotes?.map((note) => ({
          text: note.note,
          timestamp: note.timestamp,
        })),
        isLocalListing: true,
        convertedFromLeadId: leadsSeller?._id, // Track the original lead
      };
      ...
      const responseSigned = await fetch("/api/leads/seller/signed", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
        body: JSON.stringify({ _id: leadsSeller?._id, isSigned: true }),
      });

      if (!responseSigned.ok) {
        const errorData = await response.json();
        //console.log("ERROR DATA:", errorData);
        throw new Error(errorData.message || "Failed to sign Seller");
      }
      ...
      router.push(`/listings/listing-sales/${responseData.data._id}`);
    } catch (error) {
      setIsLoading(false);
                      ...
                      <Select onValueChange={field.onChange} defaultValue={field.value}>
                        <FormControl>
                          <SelectTrigger className="text-black whitespace-nowrap w-[180px]">
                            <SelectValue placeholder="Select Option text-black" />
                          </SelectTrigger>
                        </FormControl>
                        <SelectContent>
