...
import SellerForm from "./add-listing-sales-form";
...
async function fetchLeadsSeller(params: FetchSellersParams) {
  const response = await fetch("/api/listings/fetch", {
    method: "POST",
    body: JSON.stringify({ ...params, mode: "for-sale" }),
  });
...
export function DataTable<TData, TValue>({ columns }: DataTableProps<TData, TValue>) {
  ...
  const storeFilters = useSortingFiltersStore((state) => state?.filters?.listingsAgentsSales);
  ...
  const {
    ...
    queryKey: ["listingsSales", debouncedSearch],
    queryFn: () =>
      fetchLeadsSeller({
  ...
  useEffect(() => {
    if (allLeadsSellerData) {
      allLeadsSellerData.forEach((listing: ListingSales) => {
        queryClient.setQueryData(["listing-sale", listing._id], listing);
      });
    }
  ...
  const filteredData = useMemo(() => {
    ...
    if (statusFilters.length > 0) {
      data = data.filter((item: ListingSales) => statusFilters.includes(item.propertyStatus));
    }

    // Filter by search (check fullname and email)
    if (searchInput) {
      const searchLower = searchInput.toLowerCase();
      data = data.filter((item: ListingSales) => {
  ...
  const contactsTable = useReactTable({
    ...
    onSortingChange: (updaterOrValue) => {
      ...
      setSorting("listingsAgentsSales", newSorting);
    },
  ...
  const handleBulkDelete = async () => {
    ...
    try {
      ...
      const response = await fetch("/api/listings/delete", {
        ...
        body: JSON.stringify({ propertyIds: ListingSalesIds }),
      });
    ...
    <div className="relative pb-[100px]">
      <div className="flex items-center justify-between mb-4 sm:sticky top-[1px] z-1 bg-white">
        <div className="flex gap-2">
          <Badge
            ...
            onClick={() => setStatusFilters("listingsAgentsSales", [])}
          >
            All
          </Badge>
          <Badge
            ...
            onClick={() => {
              ...
              setStatusFilters("listingsAgentsSales", newFilters);
            }}
          >
            Active
          </Badge>
        ...
        <div className="flex items-center gap-4">
          ...
          {selectedCount === 0 ? (
            <FormSheet
              buttonText="Add Sales Listing"
              title="Add Listing - Sales"
              sections={sections}
      ...
      <Dialog open={bulkDeleteOpen} onOpenChange={setBulkDeleteOpen}>
        <DialogContent showCloseButton={false} className="sm:w-[500px] rounded-lg">
          <DialogHeader>
            <DialogTitle>Delete {selectedCount === 1 ? "Sales Listing" : "Sales Listings"}</DialogTitle>
          </DialogHeader>
          <DialogDescription>
            You&apos;re about to permanently delete {selectedCount} sales listing{selectedCount === 1 ? "" : "s"} and
          ...
          <DialogFooter>
            ...
            >
              {isDeletingBulk ? (
                ...
                `Delete ${selectedCount === 1 ? "Listing" : "Listings"}`
              )}
            </Button>
          </DialogFooter>
        </DialogContent>
