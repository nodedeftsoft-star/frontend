...
export default function ViewListingRentals() {
  ...
  const {
    ...
    queryKey: ["listing-rental", id],
    queryFn: () => fetchListingSale(id as string),
    enabled: !!id,
    // Use cached data immediately for instant display
    initialData: () => {
      return queryClient.getQueryData(["listing-rental", id]);
    },
  ...
  const handleLeadEditBuyerSuccess = () => {
    ...
    queryClient.invalidateQueries({ queryKey: ["listing-rental", id] });
  };
  ...
  const handleDelete = async () => {
    ...
    try {
      const response = await fetch("/api/listings/delete", {
        ...
        body: JSON.stringify({ propertyIds: [ListingsRental?._id] }),
      });
      ...
      <div className="flex justify-between w-full px-2 mb-8">
        ...
        <div className="flex gap-2 pr-12">
          ...
          <FormSheet
            ...
            icon
          >
            <EditListingRentalsForm listing={ListingsRental as ListingRentals} onSuccess={handleLeadEditBuyerSuccess} />
          </FormSheet>
          {/* <Button variant={"ghost"} size={"icon"} className="bg-transparent">
            <Link href={"/listings/listing-sales/" + ListingsRental?._id}>
                  ...
                  <div className="flex flex-col justify-between h-full">
                    <ScrollArea className="h-[325px] w-full ">
                      <div className="pb-6 mx-1">
                        {ListingsRental?.salesNote?.map((item, index) => (
