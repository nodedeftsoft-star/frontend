...
import { PhoneInput } from "@/components/ui/phone-input";
import { ListingRentals, ListingOwner } from "@/types/listings";
...
const formSchema = z.object({
  ...
  isVoucherApproved: z.boolean().default(false),
...
interface ListingRentalsFormProps {
  onSuccess?: () => void;
  previousListings?: ListingRentals[];
}

export default function ListingRentalsForm({ onSuccess, previousListings }: ListingRentalsFormProps) {
  const [isLoading, setIsLoading] = useState(false);
  const [neighborhoodSearch, setNeighborhoodSearch] = useState("");
  const { setSelectedLead } = useSelectedLeadsStore();
  const queryClient = useQueryClient();
  const { user } = useUserStore();
  const { openView } = useListingRentalStore();
  ...
  async function onSubmit(data: z.infer<typeof formSchema>) {
    try {
      ...
      const encodedData = {
        ...
        isVoucherApproved: data.isVoucherApproved || false,
        owner: {
      ...
      if (!response.ok) {
        ...
        throw new Error(errorData.message || "Failed to add Listing Rental");
      }

      const responseData = await response.json();
      setSelectedLead(responseData.data);
      queryClient.setQueryData(["listing-rental", responseData.data._id], responseData.data);
          ...
          <Card id="voucher">
            ...
            <CardContent>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <FormField
                  control={form.control}
                  name="isVoucherApproved"
                  render={({ field }) => (
                    <FormItem>
                      <FormLabel>Is Voucher Approved?</FormLabel>
                      <Select
