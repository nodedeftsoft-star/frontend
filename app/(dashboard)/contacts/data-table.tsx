...
import { useSelectedMatchStore } from "@/store/selected";
...
async function fetchLeadsBuyers(params: FetchBuyersParams) {
  const response = await fetch("/api/contacts/fetch", {
    method: "POST",
    body: JSON.stringify(params),
  });
  const data = await response.json();

  ////console.log("RENTALS MATCHES:", data);
  return data.data.contacts;
}

export function DataTable<TData, TValue>({ columns }: DataTableProps<TData, TValue>) {
  // Get persistent filters from store
  const storeFilters = useSortingFiltersStore((state) => state?.filters?.contacts);
  ...
  const { selectedMatch } = useSelectedMatchStore();
  ...
  useEffect(() => {
    if (allLeadsBuyerData) {
      allLeadsBuyerData.forEach((contact: Contact) => {
        queryClient.setQueryData(["contact", contact._id], contact);
      });
    }
  ...
  const filteredData = useMemo(() => {
    ...
    if (statusFilter) {
      filtered = filtered.filter((contact: Contact) => contact?.status.toLowerCase() === statusFilter.toLowerCase());
    }

    // Apply stage filter locally
    if (typeFilter.length > 0) {
      filtered = filtered.filter((contact: Contact) => {
        const prospectType = contact.contact.prospectType?.toLowerCase() || "";
        const contactType = contact.contactType?.toLowerCase() || "";

        return typeFilter.includes(prospectType) || typeFilter.includes(contactType);
      });
    }
  ...
  const contactsTable = useReactTable({
    ...
    onSortingChange: (updaterOrValue) => {
      ...
      setSorting("contacts", newSorting);
    },
  ...
  const handleBulkDelete = async () => {
    ...
    try {
      ...
      const contactIds = selectedRows.map((row) => (row.original as Contact)._id);
      ...
      toast.success(`Successfully deleted ${selectedRows.length} contact${selectedRows.length > 1 ? "es" : ""}`);
  ...
  return (
    <div className="relative pb-[200px] p-4 max-h-[calc(100vh-64px)]">
      <div className="flex items-center justify-between mb-4 sm:sticky top-[1px] z-1 bg-white">
        <div className="flex gap-2">
          ...
          <Badge
            ...
            onClick={() => setStatusFilter("contacts", "active")}
          >
            Active
          </Badge>
          <Badge
            ...
            onClick={() => setStatusFilter("contacts", "inactive")}
          >
            Inactive
          </Badge>
        </div>
        <div className="flex items-center gap-4">
          ...
          <Filters
            ...
            onFilterChange={(filter, checked) => {
              ...
              setTypeFilter("contacts", newFilters);
            }}
            value={typeFilter}
          />
      ...
      <Sheet open={viewLeadBuyer} onOpenChange={setViewLeadBuyer}>
        <SheetContent className="sm:w-[1264px] rounded-t-lg rounded-b-lg w-full">
          <ViewContact contact={selectedMatch as unknown as Contact} />
        </SheetContent>
      </Sheet>
      ...
      <Dialog open={bulkDeleteOpen} onOpenChange={setBulkDeleteOpen}>
        <DialogContent showCloseButton={false} className="sm:w-[500px] rounded-lg">
          <DialogHeader>
            <DialogTitle>Delete {selectedCount === 1 ? "Contact" : "Contacts"}</DialogTitle>
          </DialogHeader>
          <DialogDescription>
            You&apos;re about to permanently delete {selectedCount} contact{selectedCount === 1 ? "" : "s"} and all
