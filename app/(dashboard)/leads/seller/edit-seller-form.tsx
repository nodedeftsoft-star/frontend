...
interface LeadSellerFormProps {
  leadsSeller: LeadsSeller;
  onSuccess?: () => void;
}

export default function EditLeadsSellerForm({ onSuccess, leadsSeller }: LeadSellerFormProps) {
  ...
  const form = useForm({
    ...
    defaultValues: {
      ...
      propertyPrice: leadsSeller?.propertyPrice ? Number(leadsSeller?.propertyPrice) : 0,
      propertyAddress: leadsSeller?.propertyAddress || "",
      propertyState: leadsSeller?.propertyState || "NY",
      ...
      bedroomCount: leadsSeller?.bedroomCount?.toString(),
      bathroomCount: leadsSeller?.bathroomCount?.toString(),
  ...
  async function onSubmit(data: z.infer<typeof formSchema>) {
    try {
      ...
      const encodedData = {
        ...
        _id: leadsSeller?._id, // Use data without leadType
        petOwned: data.petOwned !== "no" ? data.petOwned : undefined,
      };

      const response = await fetch("/api/leads/seller/update", {
        method: "POST",
        headers: {
