...
export default function LeadSellerDetails() {
  const [open, setOpen] = useState(false);
  const [openDelete, setOpenDelete] = useState(false);
  const [activityNote, setActivityNote] = useState("");
  const [isSubmittingActivity, setIsSubmittingActivity] = useState(false);
  const [isSigning, setIsSigning] = useState(false);
  const [editLeadSellerOpen, setEditLeadSellerOpen] = useState(false);
  //   const [leadsSellerData, setLeadsSellerData] = useState<LeadsSeller>(leadsSeller as LeadsSeller);
  ...
  const {
    data: leadsSellerData,
    isLoading,
    isFetching,
  } = useQuery({
    queryKey: ["lead-seller", id],
    queryFn: () => fetchLeadsSeller(id as string),
    enabled: !!id,
    initialData: () => {
      return queryClient.getQueryData(["lead-seller", id]);
    },
    staleTime: 2 * 60 * 1000,
    gcTime: 10 * 60 * 1000,
  ...
  async function submitActivity() {
    try {
      ...
      const response = await fetch(`/api/leads/seller/activities/${leadsSellerData?._id}`, {
        method: "POST",
        headers: {
      ...
      queryClient.invalidateQueries({ queryKey: ["lead-seller", id] });
  ...
  const handleSignLead = async () => {
    try {
      setIsSigning(true);
      router.push("/listings/listing-sales/convert-lead" + `/${leadsSellerData?._id}`);
    } catch (error) {
      setIsSigning(false);
  ...
  const handleUpdateLeadType = async (leadType: string) => {
    try {
      ...
      const response = await fetch("/api/leads/seller/update-leadtype", {
        ...
        body: JSON.stringify({ _id: leadsSellerData?._id, leadType }),
      });
  ...
  const handleUpdateStatus = async (status: string) => {
    try {
      ...
      const response = await fetch("/api/leads/seller/update-status", {
        ...
        body: JSON.stringify({ _id: leadsSellerData?._id, status }),
      });
  ...
  const handleLeadEditSellerSuccess = () => {
    queryClient.invalidateQueries({ queryKey: ["lead-seller", id] });
    queryClient.invalidateQueries({ queryKey: ["leadsSellers"] });
    setEditLeadSellerOpen(false);
  };
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      const leadSellersIds = [leadsSellerData?._id];

      const response = await fetch("/api/leads/seller/delete", {
        method: "DELETE",
        headers: {
          "Content-Type": "application/json",
        },
        body: JSON.stringify({ leadSellersIds }),
      });
      ...
      toast.success(`Successfully deleted lead seller`);
      queryClient.invalidateQueries({ queryKey: ["leadsSellers"] });
      router.push("/leads/seller");
      setOpenDelete(false);
    } catch (_error) {
      toast.error("Failed to delete lead seller. Please try again.");
    ...
    <>
      <div className="flex justify-between flex-wrap gap-2 px-8">
        <div className="flex flex-col items-start gap-2">
          <h1 className="text-light-text text-2xl font-bold">{leadsSellerData?.propertyAddress}</h1>
          <div className="flex items-center gap-4">
            <span className="rounded-[8px] text-[#07192C] font-normal  ">
              {`${leadsSellerData?.propertyAddress || ""}, ${leadsSellerData?.propertyCounty || ""}, ${
                leadsSellerData?.propertyState?.toLocaleUpperCase() || ""
              } ${leadsSellerData?.propertyZipcode || ""}` || ""}
            </span>
        ...
        <div className="flex gap-2 pr-12">
          <Dialog open={open} onOpenChange={setOpen}>
            <DialogTrigger asChild>
              <Button size={"sm"} disabled={leadsSellerData?.isSigned}>
                {leadsSellerData?.isSigned ? "Signed" : "Sign"}
              </Button>
            </DialogTrigger>
            <DialogContent showCloseButton={false} className="sm:w-[1264px] rounded-t-lg rounded-b-lg ">
              {getMissingFields(leadsSellerData as LeadsSeller).length === 0 && (
                <>
                  <DialogHeader>
                    <DialogTitle>Signed Lead?</DialogTitle>
                  </DialogHeader>
                  <div className="space-y-3">
                    Move lead to <span className="font-bold">Listing&apos;s Agent Portal</span>
                    <Separator className="my-4" />
                    If you have signed a contract with this lead, this will convert the lead as a{" "}
                    <span className="font-bold">Sales Listing</span> in the Listing&apos;s Agent Portal.
                    <br />
                    <br />
                  ...
                  <DialogFooter>
                    ...
                    <Button
                      ...
                      disabled={leadsSellerData?.isSigned}
                    >
              ...
              {getMissingFields(leadsSellerData as LeadsSeller).length > 0 && (
                <>
                  ...
                  <DialogHeader className="gap-6">
                    ...
                    <div className="flex flex-col gap-2 text-sm text-gray-500">
                      ...
                      <ul className="list-disc pl-4 mt-2">
                        {getMissingFields(leadsSellerData as LeadsSeller).map((field, index) => (
                  ...
                  <div className="flex w-full self-auto justify-between">
                    ...
                    <SheetClose
                      onClick={() => setEditLeadSellerOpen(true)}
          ...
          <Select
            value={status}
            defaultValue={leadsSellerData?.status}
          ...
          <Select
            value={leadType}
            defaultValue={leadsSellerData?.leadType}
          ...
          <FormSheet
            ...
            sections={sections}
            open={editLeadSellerOpen}
            onOpenChange={setEditLeadSellerOpen}
            onSuccess={handleLeadEditSellerSuccess}
            icon
          >
            <EditLeadsSellerForm leadsSeller={leadsSellerData as LeadsSeller} onSuccess={handleLeadEditSellerSuccess} />
          </FormSheet>
          ...
          <Dialog open={openDelete} onOpenChange={setOpenDelete}>
            ...
            <DialogContent showCloseButton={false} className="sm:w-[1264px] rounded-t-lg rounded-b-lg ">
              <DialogHeader>
                <DialogTitle>Delete Seller Lead</DialogTitle>
              </DialogHeader>
              <DialogDescription>
        ...
        <section className="grid grid-cols-1 md:grid-cols-2 gap-4 justify-between my-8 border border-[#E5E8EB] p-6 rounded-lg shadow ">
          <div className="w-full">
            <div className="border border-[#E5E8EB] w-full flex flex-col rounded-lg p-6 mb-4">
              ...
              <div className="flex flex-col gap-4">
                {[
                  ...
                  { label: "Location", value: leadsSellerData?.propertyAddress },
                  { label: "Property Type", value: leadsSellerData?.propertyType },
                  {
                    label: "Price",
                    value: leadsSellerData?.propertyPrice?.toLocaleString("en-US", {
                      style: "currency",
                      currency: "USD",
                    }),
                  },

                  { label: "Allowed Pets", value: leadsSellerData?.petOwned || "No" },
            ...
            <div className="border border-[#E5E8EB] w-full flex flex-col rounded-lg p-6 mb-4">
              ...
              <div className="flex flex-col gap-4">
                {[
                  { label: "Name", value: leadsSellerData?.firstName + " " + leadsSellerData?.lastName },
                  { label: "Contact", value: formatPhoneToInternational(leadsSellerData?.phoneNumber || "") },
                  { label: "Email", value: leadsSellerData?.email },
                ].map(({ label, value }, idx) => (
                  <div key={idx} className="flex items-center gap-4">
                    <p className="text-light-text-secondary text-sm w-32 shrink-0">{label}</p>
            ...
            <div className="border border-[#E5E8EB] w-full flex flex-col rounded-lg p-6 mb-4">
              ...
              <div className="text-[#07192C] font-normal flex-1 ">{leadsSellerData?.description}</div>
            </div>
          </div>
              ...
              <div>
                <ScrollArea className="h-[325px] w-full">
                  <div className="pb-6 mx-1">
                    {leadsSellerData?.activityNotes.map((item, index) => (
