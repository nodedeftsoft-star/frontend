...
import { BulkImportTemplate, LeadsSeller } from "@/types/leads";
...
import SellerForm from "./add-seller-form";
...
async function fetchLeadsSeller(params: FetchSellersParams) {
  const response = await fetch("/api/leads/seller/fetch", {
    method: "POST",
    body: JSON.stringify(params),
  });
  const data = await response.json();

  //console.log("LEADS SELLER:", data);
...
export function DataTable<TData, TValue>({ columns }: DataTableProps<TData, TValue>) {
  ...
  } = useSortingFiltersStore((state) => state.filters.leadsSellers);
  ...
  const [addLeadSellerOpen, setAddLeadSellerOpen] = useState(false);
  ...
  const {
    ...
    queryKey: ["leadsSellers", debouncedSearch],
    queryFn: () =>
      fetchLeadsSeller({
  ...
  // Prime cache with individual lead seller details for instant navigation
  useEffect(() => {
    if (allLeadsSellerData) {
      allLeadsSellerData.forEach((leadSeller: LeadsSeller) => {
        queryClient.setQueryData(["lead-seller", leadSeller._id], leadSeller);
      });
    }
  ...
  const filteredData = useMemo(() => {
    ...
    // Filter by status
    if (statusFilter) {
      data = data.filter((item: LeadsSeller) => item.status === statusFilter);
    }

    // Filter by lead type
    if (typeFilter) {
      data = data.filter((item: LeadsSeller) => item.leadType === typeFilter);
    }

    // Filter by search (check fullname and email)
    if (searchInput) {
      const searchLower = searchInput.toLowerCase();
      data = data.filter((item: LeadsSeller) => {
    ...
    return data;
  }, [allLeadsSellerData, statusFilter, typeFilter, searchInput]);
  ...
  const contactsTable = useReactTable({
    ...
    onSortingChange: (updaterOrValue) => {
      ...
      setSorting("leadsSellers", newSorting);
    },
  ...
  const handleLeadAddSellerSuccess = () => {
    setAddLeadSellerOpen(false);
  };
  ...
  const handleBulkUpload = async () => {
    ...
    setIsUploading(true);
    try {
      const response = await fetch("/api/leads/seller/bulk-add", {
        method: "POST",
        headers: {
  ...
  const handleBulkDelete = async () => {
    ...
    try {
      ...
      const leadSellersIds = selectedRows.map((row) => (row.original as LeadsSeller)._id);
      ...
      toast.success(`Successfully deleted ${selectedRows.length} seller${selectedRows.length > 1 ? "s" : ""}`);
      ...
      toast.error("Failed to delete Sellers. Please try again.");
    ...
    <div className="relative pb-[100px]">
      <div className="flex items-center justify-between mb-4 sm:sticky top-[1px] z-1 bg-white">
        <div className="flex gap-2">
          <Badge
            ...
            onClick={() => setStatusFilter("leadsSellers", "")}
          >
            All
          </Badge>
          <Badge
            ...
            onClick={() => setStatusFilter("leadsSellers", "Active")}
          >
            Active
          </Badge>
          <Badge
            ...
            onClick={() => setStatusFilter("leadsSellers", "Inactive")}
          >
            Inactive
          </Badge>
        </div>
        <div className="flex items-center gap-4">
          ...
          <Filters
            ...
            onFilterChange={(filter, checked) => {
              ...
              setTypeFilter("leadsSellers", newFilters);
            }}
          ...
          <Dialog open={open} onOpenChange={handleDialogClose}>
            ...
            >
              ...
              <DialogDescription>
                Upload Contacts for: <span className="font-bold text-black">Seller</span>
              </DialogDescription>
          ...
          {selectedCount === 0 ? (
            <FormSheet
              buttonText="Add Seller Lead"
              title="Add Seller Lead"
              sections={sections}
              open={addLeadSellerOpen}
              onOpenChange={setAddLeadSellerOpen}
              onSuccess={handleLeadAddSellerSuccess}
            >
              <SellerForm />
            </FormSheet>
          ) : (
            ...
            >
              ...
              {selectedCount === 1 ? "Delete Selected Seller" : "Delete Selected Sellers"}
            </Button>
          )}
        </div>
      </div>
      ...
      <Dialog open={bulkDeleteOpen} onOpenChange={setBulkDeleteOpen}>
        <DialogContent showCloseButton={false} className="sm:w-[500px] rounded-lg">
          <DialogHeader>
            <DialogTitle>Delete {selectedCount === 1 ? "Seller Lead" : "Seller Leads"}</DialogTitle>
          </DialogHeader>
          <DialogDescription>
            You&apos;re about to permanently delete {selectedCount} seller lead{selectedCount === 1 ? "" : "s"} and all
