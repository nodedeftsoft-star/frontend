...
export default function LeadRenterDetails() {
  const [open, setOpen] = useState(false);
  const [openDelete, setOpenDelete] = useState(false);
  const [activityNote, setActivityNote] = useState("");
  const [isSubmittingActivity, setIsSubmittingActivity] = useState(false);
  const [isSigning, setIsSigning] = useState(false);
  const [editLeadRenterOpen, setEditLeadRenterOpen] = useState(false);
  //   const [leadsRenterData, setLeadsRenterData] = useState<LeadsRenter>(leadsRenter as LeadsRenter);
  ...
  const {
    data: leadsRenterData,
    isLoading,
    isFetching,
  } = useQuery({
    queryKey: ["lead-renter", id],
    queryFn: () => fetchLeadsRenter(id as string),
    enabled: !!id,
    // Use cached data immediately for instant display
    initialData: () => {
      return queryClient.getQueryData(["lead-renter", id]);
    },
  ...
  async function submitActivity() {
    try {
      ...
      const response = await fetch(`/api/leads/renter/activities/${leadsRenterData?._id}`, {
        method: "POST",
        headers: {
      ...
      queryClient.invalidateQueries({ queryKey: ["lead-renter", id] });
  ...
  const handleSignLead = async () => {
    try {
      ...
      router.push("/buyers/renter/convert-lead" + `/${leadsRenterData?._id}`);
    } catch (error) {
      setIsSigning(false);
  ...
  const handleUpdateLeadType = async (leadType: string) => {
    try {
      ...
      const response = await fetch("/api/leads/renter/update-leadtype", {
        ...
        body: JSON.stringify({ _id: leadsRenterData?._id, leadType }),
      });
      ...
      queryClient.invalidateQueries({ queryKey: ["lead-renter", leadsRenterData?._id] });
      queryClient.invalidateQueries({ queryKey: ["leadsRenters"] });
  ...
  const handleUpdateStatus = async (status: string) => {
    try {
      ...
      const response = await fetch("/api/leads/renter/update-status", {
        ...
        body: JSON.stringify({ _id: leadsRenterData?._id, status }),
      });
  ...
  const handleLeadEditRenterSuccess = () => {
    queryClient.invalidateQueries({ queryKey: ["lead-renter", id] });
    queryClient.invalidateQueries({ queryKey: ["leadsRenters"] });
    setEditLeadRenterOpen(false);
  };
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      const leadRentersIds = [leadsRenterData?._id];
  ...
  return (
    <>
      <div className="flex justify-between flex-wrap gap-2 px-8">
        <div className="flex items-center gap-4">
          <h1 className="text-light-text text-2xl font-bold">
            {leadsRenterData?.firstName} {leadsRenterData?.lastName}
          </h1>
        ...
        <div className="flex gap-2 pr-12">
          <Dialog open={open} onOpenChange={setOpen}>
            <DialogTrigger asChild>
              <Button size={"sm"} disabled={leadsRenterData?.isSigned}>
                {leadsRenterData?.isSigned ? "Signed" : "Sign"}
              </Button>
            </DialogTrigger>
            <DialogContent showCloseButton={false} className="sm:w-[1264px] rounded-t-lg rounded-b-lg ">
              {getMissingFields(leadsRenterData as LeadsRenter).length === 0 && (
                <>
                  <DialogHeader>
                  ...
                  <div className="space-y-3">
                    ...
                    <span className="font-bold">Renter</span> in the Buyer&apos;s Agent Portal.
                    <br />
                    <br />
                  ...
                  <DialogFooter>
                    ...
                    <Button
                      ...
                      disabled={leadsRenterData?.isSigned}
                    >
              ...
              {getMissingFields(leadsRenterData as LeadsRenter).length > 0 && (
                <>
                  ...
                  <DialogHeader className="gap-6">
                    ...
                    <div className="flex flex-col gap-2 text-sm text-gray-500">
                      ...
                      <ul className="list-disc pl-4 mt-2">
                        {getMissingFields(leadsRenterData as LeadsRenter).map((field, index) => (
                  ...
                  <div className="flex w-full self-auto justify-between">
                    ...
                    <SheetClose
                      onClick={() => setEditLeadRenterOpen(true)}
          ...
          <Select
            value={status}
            defaultValue={leadsRenterData?.status}
          ...
          <Select
            value={leadType}
            defaultValue={leadsRenterData?.leadType}
          ...
          <FormSheet
            ...
            sections={sections}
            open={editLeadRenterOpen}
            onOpenChange={setEditLeadRenterOpen}
            onSuccess={handleLeadEditRenterSuccess}
            icon
          >
            <EditRenterForm leadsRenter={leadsRenterData as LeadsRenter} onSuccess={handleLeadEditRenterSuccess} />
          </FormSheet>
          ...
          <div className="w-full ">
            <div className="border border-[#E5E8EB] w-full flex flex-col rounded-lg p-6 mb-4 ">
              ...
              <div className="flex flex-col gap-4">
                {[
                  { label: "Email", value: leadsRenterData?.email },
                  { label: "Mobile", value: formatPhoneToInternational(leadsRenterData?.phoneNumber || "") },
                  { label: "#Adults", value: leadsRenterData?.adultCount },
                  { label: "#Children", value: leadsRenterData?.childrenCount },
                ].map(({ label, value }, idx) => (
            ...
            <div className="border border-[#E5E8EB] w-full flex flex-col rounded-lg p-6 mb-4">
              ...
              <div className="flex flex-col gap-4">
                {[
                  { label: "HH Income/year", value: leadsRenterData?.annualHouseholdIncome || "-" },
                  {
                    label: "Max. Budget",
                    value:
                      leadsRenterData?.maxRentalPrice?.toLocaleString("en-US", {
                        style: "currency",
                        currency: "USD",
                      }) || "-",
                  },
                  { label: "Credit Score", value: leadsRenterData?.creditScore },
                ].map(({ label, value }, idx) => (
                  <div key={idx} className="flex items-center gap-4">
                    <p className="text-light-text-secondary text-sm w-fit shrink-0">{label}</p>
                    <div className="text-[#07192C] font-normal flex-1 overflow-hidden text-ellipsis whitespace-nowrap">
                ...
                {[
                  {
                    label: "County",
                    value:
                      getCountiesForNeighborhoods(leadsRenterData?.targetAreas || []).length > 0
                        ? getCountiesForNeighborhoods(leadsRenterData?.targetAreas || [])
                        : "-",
                  },
                  {
                    label: "Neighborhoods",
                    value:
                      leadsRenterData?.targetAreas.length && leadsRenterData?.targetAreas.length > 0
                        ? leadsRenterData?.targetAreas
                        : "-",
                  },
                  { label: "Property Type", value: leadsRenterData?.propertyType || "-" },

                  { label: "#Bedrooms", value: leadsRenterData?.bedroomCount || "0+" },
                  { label: "#Bathrooms", value: leadsRenterData?.bathroomCount || "0+" },
                  { label: "Pets", value: leadsRenterData?.petOwned ? `Yes, ${leadsRenterData?.petOwned}` : "No" },
                ].map(({ label, value }, idx) => (
                  <div
                    key={idx}
            ...
            <div className="border border-[#E5E8EB] w-full flex flex-col rounded-lg p-6 mb-4">
              ...
              <div className="flex flex-col gap-4">
                {[
                  {
                    label: "Laundry",
                    value: leadsRenterData?.amenities?.includes(Amenities.LaundryInUnit) ? "Yes" : "No",
                  },
                  {
                    label: "Swimming Pool",
                    value: leadsRenterData?.amenities?.includes(Amenities.SwimmingPool) ? "Yes" : "No",
                  },
                  {
                    label: "Parking",
                    value: leadsRenterData?.amenities?.includes(Amenities.ParkingSpace) ? "Yes" : "No",
                  },
                ].map(({ label, value }, idx) => (
                  <div key={idx} className="flex items-center gap-4">
                    <p className="text-light-text-secondary text-sm w-fit shrink-0">{label}</p>
              ...
              <div>
                <ScrollArea className="h-[325px] w-full">
                  <div className="pb-6 mx-1">
                    {leadsRenterData?.activityNotes.map((item, index) => (
