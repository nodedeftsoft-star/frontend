...
import { BulkImportTemplate, LeadsRenter } from "@/types/leads";
import { useDebounce } from "@/hooks/useDebounce";
import { parseExcelBuyerFile, validateFile } from "@/lib/bulkUpload";
import { toast } from "sonner";
import Filters from "@/components/ui/filters";
import FormSheet from "@/components/ui/form-sheet";
import AddRenterForm from "./add-renter-form";
...
export const sections = [
  ...
  { id: "renternotes", name: "Renter Notes" },
];

type FetchRentersParams = {
  search: string;
  status: string;
  type: string;
};

async function fetchLeadsRenters(params: FetchRentersParams) {
  const response = await fetch("/api/leads/renter/fetch", {
    method: "POST",
    headers: {
...
export function DataTable<TData, TValue>({ columns }: DataTableProps<TData, TValue>) {
  ...
  } = useSortingFiltersStore((state) => state.filters.leadsRenters);
  ...
  const [addLeadRenterOpen, setAddLeadRenterOpen] = useState(false);
  ...
  const {
    data: allLeadsRenterData,
    isLoading: isLeadsRentersLoading,
    isFetching,
  } = useQuery({
    queryKey: ["leadsRenters", debouncedSearch],
    queryFn: () =>
      fetchLeadsRenters({
  ...
  // Prime cache with individual lead renter details for instant navigation
  useEffect(() => {
    if (allLeadsRenterData) {
      allLeadsRenterData.forEach((leadRenter: LeadsRenter) => {
        queryClient.setQueryData(["lead-renter", leadRenter._id], leadRenter);
      });
    }
  }, [allLeadsRenterData, queryClient]);
  ...
  const filteredData = useMemo(() => {
    let data = allLeadsRenterData || [];

    // Filter by status
    if (statusFilter) {
      data = data.filter((item: LeadsRenter) => item.status === statusFilter);
    }

    // Filter by lead type
    if (typeFilter) {
      data = data.filter((item: LeadsRenter) => item.leadType === typeFilter);
    }

    // Filter by search (check fullname and email)
    if (searchInput) {
      const searchLower = searchInput.toLowerCase();
      data = data.filter((item: LeadsRenter) => {
    ...
    return data;
  }, [allLeadsRenterData, statusFilter, typeFilter, searchInput]);
  ...
  const contactsTable = useReactTable({
    ...
    onSortingChange: (updaterOrValue) => {
      ...
      setSorting("leadsRenters", newSorting);
    },
  ...
  const handleLeadAddRenterSuccess = () => {
    setAddLeadRenterOpen(false);
  };
  ...
  const handleBulkUpload = async () => {
    ...
    setIsUploading(true);
    try {
      const response = await fetch("/api/leads/renter/bulk-add", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
  ...
  const handleBulkDelete = async () => {
    ...
    try {
      ...
      const leadRentersIds = selectedRows.map((row) => (row.original as LeadsRenter)._id);
      ...
      toast.error("Failed to delete renters. Please try again.");
  ...
  if (isLeadsRentersLoading && !searchInput) {
    ...
    <div className="relative pb-[100px]">
      <div className="flex items-center justify-between mb-4 sm:sticky top-[1px] z-1 bg-white">
        <div className="flex gap-2">
          <Badge
            ...
            onClick={() => setStatusFilter("leadsRenters", "")}
          >
            All
          </Badge>
          <Badge
            ...
            onClick={() => setStatusFilter("leadsRenters", "Active")}
          >
            Active
          </Badge>
          <Badge
            ...
            onClick={() => setStatusFilter("leadsRenters", "Inactive")}
          >
            Inactive
          </Badge>
        </div>
        <div className="flex items-center gap-4">
          ...
          <Filters
            ...
            onFilterChange={(filter, checked) => {
              ...
              setTypeFilter("leadsRenters", newFilters);
            }}
          ...
          <Dialog open={open} onOpenChange={handleDialogClose}>
            ...
            >
              ...
              <DialogDescription>
                Upload Contacts for: <span className="font-bold text-black">Renter</span>
              </DialogDescription>
          ...
          {selectedCount === 0 ? (
            <FormSheet
              ...
              title="Add Renter"
              sections={sections}
              open={addLeadRenterOpen}
              onOpenChange={setAddLeadRenterOpen}
              onSuccess={handleLeadAddRenterSuccess}
            >
              <AddRenterForm onSuccess={handleLeadAddRenterSuccess} />
            </FormSheet>
          ) : (
            <Button
      ...
      <Dialog open={bulkDeleteOpen} onOpenChange={setBulkDeleteOpen}>
        <DialogContent showCloseButton={false} className="sm:w-[500px] rounded-lg">
          <DialogHeader>
            <DialogTitle>Delete {selectedCount === 1 ? "Renter Lead" : "Renter Leads"}</DialogTitle>
          </DialogHeader>
          <DialogDescription>
            You&apos;re about to permanently delete {selectedCount} renter lead{selectedCount === 1 ? "" : "s"} and all
