...
interface LeadsLandlordProps {
  onSuccess?: () => void;
}

export default function LandlordForm({ onSuccess }: LeadsLandlordProps) {
  const [isLoading, setIsLoading] = useState(false);
  const [neighborhoodSearch, setNeighborhoodSearch] = useState("");
  const { setSelectedLead } = useSelectedLeadsStore();
  const queryClient = useQueryClient();
  const { openView } = useLeadsLandlordStore();
  ...
  async function onSubmit(data: z.infer<typeof formSchema>) {
    try {
      ...
      const response = await fetch("/api/leads/landlord/add", {
        method: "POST",
        headers: {
      ...
      queryClient.setQueryData(["lead-landlord", responseData.data._id], responseData.data);
