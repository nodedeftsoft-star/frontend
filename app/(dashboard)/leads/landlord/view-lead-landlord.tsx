...
export default function ViewLeadLandlord({
  leadsLandlord,
  closeAll,
}: {
  leadsLandlord: LeadsLandLord | undefined | null;
  closeAll?: () => void;
}) {
  ...
  const [leadType, setLeadType] = useState<string>(leadsLandlord?.leadType || "");
  const [status, setStatus] = useState<string>(leadsLandlord?.status || "");
  const [isUpdatingleadType, setIsUpdatingleadType] = useState(false);
  const [isUpdatingStatus, setIsUpdatingStatus] = useState(false);
  const [editOpen, setEditOpen] = useState(false);
  // const [leadCheckModal, setLeadCheckModal] = useState(false);
  ...
  const handleEditSuccess = () => {
    setEditOpen(false);
    queryClient.invalidateQueries({ queryKey: ["lead-landlord", leadsLandlord?._id] });
  ...
  async function submitActivity() {
    try {
      ...
      const response = await fetch(`/api/leads/landlord/activities/${leadsLandlord?._id}`, {
        method: "POST",
        headers: {
  ...
  const handleSignLead = async () => {
    try {
      setIsSigning(true);
      router.push("/listings/listing-rentals/convert-lead" + `/${leadsLandlord?._id}`);
    } catch (error) {
      setIsSigning(false);
  ...
  const handleUpdateLeadType = async (leadType: string) => {
    try {
      ...
      const response = await fetch("/api/leads/landlord/update-leadtype", {
        ...
        body: JSON.stringify({ _id: leadsLandlord?._id, leadType }),
      });
    ...
    <>
      <div className="flex justify-between  px-8">
        <div className="flex flex-col items-start gap-2">
          <h1 className="text-light-text text-2xl font-bold">{leadsLandlord?.propertyAddress}</h1>
          <div className="flex items-center gap-4">
            {leadsLandlord?.propertyAddress && leadsLandlord?.propertyNeighborhood ? (
              <span className="rounded-[8px] text-[#07192C] font-normal  ">
                {`${leadsLandlord?.propertyAddress}, ${
                  leadsLandlord?.propertyCounty
                }, ${leadsLandlord?.propertyState?.toLocaleUpperCase()} ${leadsLandlord?.propertyZipcode}` || ""}
              </span>
            ) : (
              " "
            )}
        ...
        <div className="flex gap-2 pr-12">
          <Dialog open={open} onOpenChange={setOpen}>
            <DialogTrigger asChild>
              <Button size={"sm"} disabled={leadsLandlord?.isSigned}>
                {leadsLandlord?.isSigned ? "Signed" : "Sign"}
              </Button>
            </DialogTrigger>
            <DialogContent showCloseButton={false} className="sm:w-[1264px] rounded-t-lg rounded-b-lg ">
              {getMissingFields(leadsLandlord as LeadsLandLord).length === 0 && (
                <>
                  <DialogHeader>
                  ...
                  <DialogFooter>
                    ...
                    <Button
                      ...
                      disabled={leadsLandlord?.isSigned}
                    >
                  ...
                  <DialogHeader className="gap-6">
                    ...
                    <div className="flex flex-col gap-2 text-sm text-gray-500">
                      ...
                      <ul className="list-disc pl-4 mt-2">
                        {getMissingFields(leadsLandlord as LeadsLandLord).map((field, index) => (
          ...
          {/* <Button onClick={() => setLeadCheckModal(true)}>test</Button> */}
          {/* ======== */}
          {/* <Dialog open={leadCheckModal} onOpenChange={setLeadCheckModal}>
            <DialogClose />
          ...
          <Select value={status} onValueChange={(e) => handleUpdateStatus(e)} disabled={isUpdatingStatus}>
            <SelectTrigger size="sm" className="bg-[#F9F9F9]">
              <SelectValue placeholder="Active" defaultValue={leadsLandlord?.status} />
            </SelectTrigger>
            <SelectContent>
              <SelectGroup>
          ...
          <Select value={leadType} onValueChange={(e) => handleUpdateLeadType(e)} disabled={isUpdatingleadType}>
            <SelectTrigger size="sm" className="bg-[#F9F9F9]">
              <SelectValue placeholder="Hot" defaultValue={leadsLandlord?.leadType} />
            </SelectTrigger>
            <SelectContent>
              <SelectGroup>
          ...
          <FormSheet
            ...
            icon
          >
            <EditLeadsLandlordForm leadsLandlord={leadsLandlord as LeadsLandLord} onSuccess={handleEditSuccess} />
          </FormSheet>

          <Button variant={"ghost"} size={"icon"} className="bg-transparent" onClick={closeAll}>
            <Link href={"/leads/landlord/" + leadsLandlord?._id}>
          ...
          <div className="w-full">
            <div className="border border-[#E5E8EB] w-full flex flex-col rounded-lg p-6 mb-4">
              ...
              <div className="flex flex-col gap-4">
                {[
                  { label: "Listing Type", value: "Rental", highlight: true },
                  { label: "Location", value: leadsLandlord?.propertyAddress },
                  { label: "Property Type", value: leadsLandlord?.propertyType },
                  {
                    label: "Monthly Rent",
                    value: leadsLandlord?.propertyPrice?.toLocaleString("en-US", {
                      style: "currency",
                      currency: "USD",
                    }),
                  },

                  { label: "Allowed Pets", value: leadsLandlord?.petOwned || "No" },
            ...
            <div className="border border-[#E5E8EB] w-full flex flex-col rounded-lg p-6 mb-4">
              ...
              <div className="flex flex-col gap-4">
                {[
                  { label: "Name", value: leadsLandlord?.firstName + " " + leadsLandlord?.lastName },
                  { label: "Contact", value: formatPhoneToInternational(leadsLandlord?.phoneNumber || "") },
                  { label: "Email", value: leadsLandlord?.email },
                ].map(({ label, value }, idx) => (
                  <div key={idx} className="flex items-center gap-4">
                    <p className="text-light-text-secondary text-sm w-32 shrink-0">{label}</p>
            ...
            <div className="border border-[#E5E8EB] w-full flex flex-col rounded-lg p-6 mb-4">
              ...
              <div className="text-[#07192C] font-normal flex-1 ">{leadsLandlord?.description}</div>
            </div>
          </div>
              ...
              <div>
                <ScrollArea className="h-[325px] w-full">
                  <div className="pb-6 mx-1">
                    {leadsLandlord?.activityNotes?.map((item, index) => (
