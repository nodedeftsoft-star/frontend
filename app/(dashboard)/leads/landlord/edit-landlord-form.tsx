...
interface LeadLandLordFormProps {
  leadsLandlord: LeadsLandLord;
  onSuccess?: () => void;
}

export default function EditLeadsLandlordForm({ onSuccess, leadsLandlord }: LeadLandLordFormProps) {
  ...
  const form = useForm({
    ...
    defaultValues: {
      leadType: leadsLandlord?.leadType,
      ...
      propertyPrice: leadsLandlord?.propertyPrice ? Number(leadsLandlord?.propertyPrice) : 0,
      propertyAddress: leadsLandlord?.propertyAddress || "",
      propertyState: leadsLandlord?.propertyState || "NY",
      ...
      bedroomCount: leadsLandlord?.bedroomCount?.toString(),
      bathroomCount: leadsLandlord?.bathroomCount?.toString(),
  ...
  async function onSubmit(data: z.infer<typeof formSchema>) {
    try {
      ...
      const encodedData = {
        ...
        _id: leadsLandlord?._id, // Use data without leadType
        petOwned: data.petOwned !== "no" ? data.petOwned : undefined,
      };

      const response = await fetch("/api/leads/landlord/update", {
        method: "POST",
        headers: {
      ...
      toast("Error", {
        description:
          error instanceof Error && error.message.includes("already")
            ? "Landlord with this email or phone number already exists"
            : "Failed to edit leads landlord",
