...
import LandlordForm from "./add-landlord-form";
...
import { BulkImportTemplate, LeadsLandLord } from "@/types/leads";
import { useDebounce } from "@/hooks/useDebounce";
import { parseExcelFile, validateFile } from "@/lib/bulkUpload";
...
type FetchLandlordsParams = {
  search: string;
  status: string;
  type: string;
};

async function fetchLeadsLandLord(params: FetchLandlordsParams) {
  const response = await fetch("/api/leads/landlord/fetch", {
    method: "POST",
    body: JSON.stringify(params),
  });
  const data = await response.json();

  // //console.log("LEADS LANDLORD:", data);
  return data.data.landlords;
}
...
export function DataTable<TData, TValue>({ columns }: DataTableProps<TData, TValue>) {
  ...
  } = useSortingFiltersStore((state) => state.filters.leadsLandlords);
  ...
  const [addLeadLandlordOpen, setAddLeadLandlordOpen] = useState(false);
  ...
  const {
    data: allLeadsLandlordData,
    isLoading: isLeadsLandlordsLoading,
    isFetching,
  } = useQuery({
    queryKey: ["leadsLandlords", debouncedSearch],
    queryFn: () =>
      fetchLeadsLandLord({
  ...
  // Prime cache with individual lead landlord details for instant navigation
  useEffect(() => {
    if (allLeadsLandlordData) {
      allLeadsLandlordData.forEach((leadLandlord: LeadsLandLord) => {
        queryClient.setQueryData(["lead-landlord", leadLandlord._id], leadLandlord);
      });
    }
  }, [allLeadsLandlordData, queryClient]);
  ...
  const filteredData = useMemo(() => {
    let data = allLeadsLandlordData || [];

    // Filter by status
    if (statusFilter) {
      data = data.filter((item: LeadsLandLord) => item.status === statusFilter);
    }

    // Filter by lead type
    if (typeFilter) {
      data = data.filter((item: LeadsLandLord) => item.leadType === typeFilter);
    }

    // Filter by search (check fullname and email)
    if (searchInput) {
      const searchLower = searchInput.toLowerCase();
      data = data.filter((item: LeadsLandLord) => {
    ...
    return data;
  }, [allLeadsLandlordData, statusFilter, typeFilter, searchInput]);
  ...
  const contactsTable = useReactTable({
    ...
    onSortingChange: (updaterOrValue) => {
      ...
      setSorting("leadsLandlords", newSorting);
    },
  ...
  const handleLeadAddLandlordSuccess = () => {
    setAddLeadLandlordOpen(false);
  };
  ...
  const handleBulkUpload = async () => {
    ...
    setIsUploading(true);
    try {
      const response = await fetch("/api/leads/landlord/bulk-add", {
        method: "POST",
        headers: {
  ...
  const handleBulkDelete = async () => {
    ...
    try {
      ...
      const leadLandlordsIds = selectedRows.map((row) => (row.original as LeadsLandLord)._id);
      ...
      toast.success(`Successfully deleted ${selectedRows.length} landlord${selectedRows.length > 1 ? "s" : ""}`);
      ...
      toast.error("Failed to delete landlords. Please try again.");
  ...
  if (isLeadsLandlordsLoading && !searchInput) {
    ...
    <div className="relative pb-[100px]">
      <div className="flex items-center justify-between mb-4 sm:sticky top-[1px] z-1 bg-white">
        <div className="flex gap-2">
          <Badge
            ...
            onClick={() => setStatusFilter("leadsLandlords", "")}
          >
            All
          </Badge>
          <Badge
            ...
            onClick={() => setStatusFilter("leadsLandlords", "Active")}
          >
            Active
          </Badge>
          <Badge
            ...
            onClick={() => setStatusFilter("leadsLandlords", "Inactive")}
          >
            Inactive
          </Badge>
        </div>
        <div className="flex items-center gap-4">
          ...
          <Filters
            ...
            onFilterChange={(filter, checked) => {
              ...
              setTypeFilter("leadsLandlords", newFilters);
            }}
          ...
          <Dialog open={open} onOpenChange={handleDialogClose}>
            ...
            >
              ...
              <DialogDescription>
                Upload Contacts for: <span className="font-bold text-black">Landlord</span>
              </DialogDescription>
              ...
              {!uploadedFile && (
                <div className="mb-4 p-3 rounded-lg flex justify-center">
                  <Button variant="outline" size="sm" asChild className="flex items-center gap-2">
                    <a href="/templates/import_seller_landlord.xlsx" download="import_seller_landlord.xlsx">
          ...
          {selectedCount === 0 ? (
            <FormSheet
              ...
              title="Add Landlord"
              sections={sections}
              open={addLeadLandlordOpen}
              onOpenChange={setAddLeadLandlordOpen}
              onSuccess={handleLeadAddLandlordSuccess}
            >
              <LandlordForm />
            </FormSheet>
          ) : (
            ...
            >
              ...
              {selectedCount === 1 ? "Delete Selected Landlord" : "Delete Selected Landlords"}
            </Button>
          )}
        </div>
      </div>
      ...
      <Dialog open={bulkDeleteOpen} onOpenChange={setBulkDeleteOpen}>
        <DialogContent showCloseButton={false} className="sm:w-[500px] rounded-lg">
          <DialogHeader>
            <DialogTitle>Delete {selectedCount === 1 ? "Landlord Lead" : "Landlord Leads"}</DialogTitle>
          </DialogHeader>
          <DialogDescription>
            You&apos;re about to permanently delete {selectedCount} landlord lead{selectedCount === 1 ? "" : "s"} and
            all their associated information. This action cannot be undone.
            <br />
            <br />
          ...
          <DialogFooter>
            ...
            >
              {isDeletingBulk ? (
                ...
                `Delete ${selectedCount === 1 ? "Landlord" : "Landlords"}`
              )}
            </Button>
          </DialogFooter>
        </DialogContent>
