...
export default function MatchFinderRenter(Data: { Data: Renter }) {
  ...
  const { closeAll } = useBuyersRenterStore();
  ...
  const updateMatch = async (propertyId: string, matchKind: string) => {
    // console.log("Updating match:", propertyId, matchKind, params.id, Data.Data._id);
    try {
      const response = await fetch("/api/renters/update-match", {
        method: "POST",
        headers: {
      ...
      console.log("QUERY KEY:", ["renter", params.id || Data.Data._id]);

      const renterId = params.id || Data.Data._id;
      queryClient.setQueryData(["renter", renterId], (oldData: Renter | undefined) => {
